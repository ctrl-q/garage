...
def test_meta_evaluator():
    ...
    with tempfile.TemporaryDirectory() as log_dir_name:
        ...
        assert len(rows) == 2
        assert float(rows[0]['MetaTest/Unknown Task/CompletionRate']) < 1.0
        assert float(rows[0]['MetaTest/Unknown Task/Iteration']) == 0
        assert (float(rows[0]['MetaTest/Unknown Task/MaxReturn']) >= float(
            rows[0]['MetaTest/Unknown Task/AverageReturn']))
        assert (float(rows[0]['MetaTest/Unknown Task/AverageReturn']) >= float(
            rows[0]['MetaTest/Unknown Task/MinReturn']))
        assert float(rows[1]['MetaTest/Unknown Task/Iteration']) == 1
