...
class GaussianMLPBaseline(Baseline):
    ...
    def __init__(self,
        ...
        self._return_dist = dict(mean=np.zeros(output_dim),
                                 std=np.ones(output_dim))
    ...
    def fit(self, paths):
        ...
        normalized_obs = torch.Tensor(
            (observations - self._obs_dist['mean']) / self._obs_dist['std'])
        normalized_return = torch.Tensor(
            (returns - self._return_dist['mean']) / self._return_dist['std'])
    ...
    def predict(self, path):
        ...
        """
        normalized_obs = torch.Tensor(
            (path['observations'] - self._obs_dist['mean']) /
            self._obs_dist['std'])
        with torch.no_grad():
            dist = self._module(normalized_obs)
        normalized_mean = dist.mean.flatten().numpy()
        mean = normalized_mean * self._return_dist['std'] + self._return_dist[
            'mean']
        return mean.flatten()
